...
Web category IDs are passed through unchanged. For example, the portal was observed sending `AlcoholAndTobacco`.
...
- `priority` (Number) The rule priority. Lower numbers are evaluated before higher numbers.
...
- `custom_headers` (Attributes List) Custom response headers to add for allow rules. Microsoft Graph accepts these only when `action` is `allow`. (see [below for nested schema](#nestedatt--custom_headers))
...
- `urls_or_fqdns` (Set of String) URL or FQDN destination patterns for the rule, for example `*.example.com`.
